...
{
    public partial class pending_buyer : System.Web.UI.Page
    {
        private BaseClass func;
        private Alert alert;
        private UserListGateway userListGateway;

        public pending_buyer()
        {
...
FROM            UserList INNER JOIN
                         ...
                         District ON UserList.District=District.DISTRICTID  WHERE UserList.Status='W' AND UserList.Type='Buyer' ORDER BY UserList.UserId ASC";
...
FROM            UserList INNER JOIN
                         Division ON UserList.Division=Division.ID INNER JOIN
                         District ON UserList.District=District.DISTRICTID  WHERE UserList.Status='W' AND UserList.Type='Buyer' AND UserList.NAME LIKE '%" + txtSearch.Text + "%'  ORDER BY UserList.UserId ASC";
                func.LoadGrid(profileGridView, query);
            }
            else
            {
