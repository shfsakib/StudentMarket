...
{
    public partial class restricted_admin : System.Web.UI.Page
    {
        private AdminGateway adminGateway;
        private BaseClass func;
        private Alert alert;
        private AdminModel adminModel;
        public restricted_admin()
        {
        ...
        {
            adminGridView.DataSource = adminGateway.GetAllRestrictAdmin();
        ...
        protected void txtSearch_OnTextChanged(object sender, EventArgs e)
        {
            if (txtSearch.Text != "")
            {
                adminGridView.DataSource = adminGateway.GetRestrictAdminByData(txtSearch.Text);
                adminGridView.DataBind();
            }
            else
            {
        ...
        protected void restrictLinkButton_OnClick(object sender, EventArgs e)
        {
            if (func.CheckType() == "Super Admin")
            {
                ...
                if (a > 0)
                {
                    func.Alert(this, "Activate Successfully", "s", false);
                    AdminData();
                }
                else
                {
                    func.Alert(this, "Activate Failed", "e", false);
                }
            }
            else
            {
